...
const MemberForgotPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      toast.success('If the phone exists, an OTP has been sent.');
    } catch (error) {
        ...
        <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl shadow-2xl relative z-10">
          {success ? (
            <div className="text-center py-6">
              ...
              <p className="text-slate-600 mb-8">We've sent an OTP to your registered phone. It expires in 30 minutes.</p>
              <Link to="/member-reset-password" className="inline-flex items-center text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                <ArrowLeft className="w-4 h-4 mr-2" /> Enter OTP and Reset Password
              </Link>
            </div>
          ) : (
            <>
              <div className="text-center">
                ...
                <p className="mt-2 text-sm text-slate-500">Enter the phone number used to register and receive an OTP.</p>
              </div>
